...
getCleannote.addEventListener('click', function(){
    if (confirm("Agree Clean Note?") == true) {
      const db = getDatabase();
      const noteRef = ref(db, 'name-noted/');
      set(noteRef, null)
    } else {
...
onValue(ref(db, 'name-noted/'), (snapshot) => {
    ...
    for (var note in data) {
      ...
      const tr = `
      <input type="text" class="form-control" readonly style="width: 82.5%; margin-bottom: 10px;border-color: #6495ED;" id="lists" value="${data[note].name}"/><i data-id="${note}" class="fa fa-remove " role="button" id="remove-note"></i>
      `
